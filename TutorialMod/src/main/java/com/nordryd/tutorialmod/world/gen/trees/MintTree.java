...
public class MintTree extends WorldGenAbstractTree
{
	public static final IBlockState LOG = ModBlocks.LOGS.getDefaultState().withProperty(Logs.VARIANT, EnumHandler.Flavors.MINT);
	public static final IBlockState LEAF = ModBlocks.LEAVES.getDefaultState().withProperty(Leaves.VARIANT, EnumHandler.Flavors.MINT);
		...
		if (!flag) { // Insufficient space for tree to grow. Terminate.
			...
			if (isSoil && (y < world.getHeight() - height - 1)) {
				...
				for (int yPos = y - 3 + height; yPos <= y + height; yPos++) {
					...
					int b2 = 1 - (b1 / 2);
					//System.out.printf("yPos = %d, b1 = %d, b2 = %d", yPos, b1, b2);
